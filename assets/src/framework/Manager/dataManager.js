...
let DataManager = {
    ...
    loadConfigs:function(progressCb, callback) {
        let CSV = {
            ...
            STANDARD_ENCODE_OPTS: {
                delimiter: DataManager.CELL_DELIMITERS[0],
                newline: DataManager.LINE_DELIMITERS[0],
                skip: 0,
                limit: false,
                header: false
            },
            ...
            parse: function(text, opts, fn) {
                ...
                if (!opts.delimiter || !opts.newline) {
                    ...
                    opts.delimiter = opts.delimiter || this.mostFrequent(text, DataManager.CELL_DELIMITERS, limit);
                    opts.newline = opts.newline || this.mostFrequent(text, DataManager.LINE_DELIMITERS, limit);
                }
        ...
        function addTable(tableName, tableContent, force) {
            if (DataManager.csvTables[tableName] && !force) {
                return;
            }
            ...
            DataManager.tableCast[tableName] = CSV.opts.cast;
            DataManager.tableComment[tableName] = CSV.opts.comment;
            DataManager.csvTables[tableName] = tableData;
            DataManager.csvTableForArr[tableName] = tableArr;
        };
        ...
        function loadPageAnimation(){
            cc.loader.loadResDir("pageAniClips", cc.AnimationClip, function(err, clips) {
                if (err) {
                    cc.error(err.message || err);
                    return;
                }
                for (var i = 0; i < clips.length; i++) {
                    DataManager.PageAnimation[clips[i].name] = clips[i];
                }
            });
        };
        ...
        function loadTableData(){
            var currentLoad = 0;
            DataManager.arrTables.forEach(function(tableName, index) {
                cc.loader.loadRes("data/" + tableName, function(err, content) {
                    ...
                    if (progressCb) {
                        progressCb(index + 1, DataManager.arrTables.length);
                    }
                    addTable(tableName, content);
                    if (callback) {
                        currentLoad++;
                        if (currentLoad >= DataManager.arrTables.length) {
                            callback();
                        }
                    }
                });
