...
cc.Class({
    ...
    Init:function(){
        ...
        this.node.on(cc.Node.EventType.MOUSE_DOWN,this.stageDowd,this);
    ...
    stageDowd:function(e){
        ...
        this.node.on(cc.Node.EventType.MOUSE_UP,this.stageUp,this);
        this.node.on(cc.Node.EventType.MOUSE_MOVE,this.stageMove,this);
    },
    stageUp:function(e){
        this.node.off(cc.Node.EventType.MOUSE_UP,this.stageUp,this);
        this.node.off(cc.Node.EventType.MOUSE_MOVE,this.stageMove,this);
        this.moveType = 0;
        console.time("Serch time=");
        this.search();
        console.timeEnd("Serch time=");
    ...
    update (dt) {
        if(this.path != null){
            if (this.path.length > 1 && this.startIndex < this.path.length){
                let a = this.path[this.startIndex].x;
                let i = this.path[this.startIndex].y;
                this.fillRect(cc.color(255,0,0),this.gx * a , this.gx * i , this.gx, this.gx);
                this.g.stroke();
                this.startIndex++;
            }
        }
